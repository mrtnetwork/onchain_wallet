...
class ZcashAccountSyncView extends StatelessWidget {
  const ZcashAccountSyncView({super.key});
  @override
  Widget build(BuildContext context) {
    return NetworkAccountControllerView<ZcashNetworkClient, IZcashAddress, ZcashChain>(
      ...
      childBulder: (wallet, account, client, address) =>
          _ZcashAccountSyncView(wallet: wallet, chain: account, client: client),
    );
  }
}

class _ZcashAccountSyncView extends StatefulWidget {
  const _ZcashAccountSyncView(
  ...
  @override
  State<_ZcashAccountSyncView> createState() => _ZcashAccountSyncViewState();
}

class _ZcashAccountSyncViewState extends ZcashAccountState<_ZcashAccountSyncView> {
  @override
  ZcashChain get account => widget.chain;
  @override
  ZcashNetworkClient get client => widget.client;
  late ZcashSyncTrackerController tracker;
  final StreamPageProgressController progressKey =
      StreamPageProgressController(initialStatus: StreamWidgetStatus.progress);
  late _SyncTracker defaultTracker;
  ZcashSyncChain syncChain = ZcashSyncChain.none;
  bool get syncIsActive => syncChain.network == account.network.coinParam.network;
  StreamSubscription<ChainEvent>? listener;
  StreamSubscription<ZcashChainNotify>? syncEvent;
  ...
  void updateSyncNetwork() async {
    context.to(PageRouter.zcashUpdateSyncNetwork);
  }

  _SyncTracker createViewRequest({
    required ZcashSyncTracker tracker,
    ...
    for (final i in defaultAddresses) {
      final addr = account.fromReceiverSync(i.index);
      final protocolAddr = addr?.account.toProtocolAddress(i.index.protocol);
      if (addr == null || protocolAddr == null) continue;
      final addrInfo = account.getOrCreateReceiptFromNetworkAddressSync(account: addr);
      syncAddresses.add(_SyncedAddressWithHeight(
          height: i.startHeight,
          address: addrInfo.copyWith(
              view: protocolAddr.address, type: i.index.protocol.name.tr)));
    }
  ...
  void onSyncingEvent(ZcashChainNotify event) {
    for (final i in requests) {
  ...
  void onInitOnce() {
    ...
    listener = account.stream.where((e) {
      if (e.type == ZcashChainNotify.trackerAccountChanged) return true;
      return false;
...
class _SyncOffsetStatus with Equality {
  ...
  factory _SyncOffsetStatus.fromOffset(BlockTrackingOffset offset) {
    ...
    return _SyncOffsetStatus(
      ...
      rangeStr: "${offset.startHeight.toInt()}/${offset.endHeight.toInt()}",
    );
  }
  factory _SyncOffsetStatus.fromTracker(ZcashSyncTracker tracker) {
    ...
    return _SyncOffsetStatus(
      ...
      rangeStr:
          "${tracker.startHeight.toInt()} - ${tracker.endHeight.toInt()}${switch (tracker.phase) {
        ZcashSyncRequestPhase.utxos => " (at ${tracker.currentHeight.toInt()})",
        ZcashSyncRequestPhase.nullifiers => " (at ${tracker.endHeight.toInt()})",
      }}",
    );
  }
...
class _SyncOffset with Equality {
  ...
  ZcashSyncTracker get tracker => syncOffset.tracker;
  final ZcashTrackingOffsetWithStatus syncOffset;
...
class _SyncTracker with DisposableMixin, StreamStateController {
  ...
  void onSyncingEvent(ZcashChainNotify event) {
    ...
    if (event == ZcashChainNotify.trackerOffsetChanged) {
