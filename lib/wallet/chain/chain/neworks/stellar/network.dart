...
class StellarNetworkController extends NetworkController<IStellarAddress, StellarChain,
    Web3StellarChainAccount, Web3InternalDefaultChain, ChainConfig> {
  StellarNetworkController({super.networks, required super.id, required super.database})
      : super(type: NetworkType.stellar);

  @override
  Future<IResult<Web3StellarChainAuthenticated>> createWeb3ChainAuthenticated(
    ...
    return internalNetwork.andThenAsync((internalNetwork) async {
      ...
      for (final i in internalNetwork.networks) {
        ...
        web3Accounts.addAll(addresses.map((e) => Web3StellarChainAccount.fromChainAccount(
            address: e,
            isDefault: e == defaultAddress,
            id: e.network.value,
            network: network.network.coinParam.stellarChainType)));
      }
      return ResultOk(Web3StellarChainAuthenticated(
