...
enum CryptoRequestHashingType {
  ...
  static CryptoRequestHashingType fromName(String? name) {
    return values.firstWhere((e) => e.name == name,
        orElse: () => throw AppInternalError.internalError("CryptoRequestHashingType"));
  }
}

class CryptoRequestHashing extends CryptoRequest<AppSerializationBinary> {
  final CryptoRequestHashingType hashingType;
  final String? dataHex;
  final List<int>? dataBytes;
  CryptoRequestHashing._({required this.hashingType, this.dataHex, List<int>? dataBytes})
  ...
  factory CryptoRequestHashing(
    ...
    if (dataHex != null && dataBytes != null) {
      throw AppInternalError.internalError("CryptoRequestHashing");
    }
    if ((dataHex == null && dataBytes == null) &&
  ...
  factory CryptoRequestHashing.deserialize({List<int>? bytes, CborObject? object}) {
    final CborListValue values = AppSerialization.decodeTaggedValue(
        cborBytes: bytes,
        cborObject: object,
        identifier: CryptoRequestMethod.hashing.tag);
    return CryptoRequestHashing._(
